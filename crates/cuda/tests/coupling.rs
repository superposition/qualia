...
//! in-strength. The arithmetic is host-side data (D-001), but the symbol hangs
//! off a layer's context, so each test opens one and reports and returns where
//! no CUDA device is present, exactly as the other device tests here do.
...
fn coupling_is_noop_without_feature() {
    ...
    assert_eq!(couple_prior(&context, &prior, &[]).expect("empty mapping"), 0.0);
    assert_eq!(
        couple_prior(&context, &prior, &[(1, 0), (0, 1)]).expect("mapped slots"),
        0.0
    );
    assert_eq!(
        couple_prior(&context, &prior, &[(9, STATE_DIM)]).expect("unmappable slots"),
        0.0
    );
}
...
fn coupling_applies_the_prior_by_type_strength() {
    ...
    assert_eq!(couple_prior(&context, &prior, &[(1, 0)]).expect("strongest type"), 1.0);
    assert_eq!(couple_prior(&context, &prior, &[(0, 0)]).expect("half strength"), 0.5);
    assert_eq!(couple_prior(&context, &prior, &[(2, 0)]).expect("quarter strength"), 0.25);
    assert_eq!(
        couple_prior(&context, &prior, &[(1, 0), (0, 1), (2, 2)]).expect("three slots"),
        1.75
    );
    // The slot a type feeds does not change the weight it carries.
    assert_eq!(
        couple_prior(&context, &prior, &[(1, 7)]).expect("a distant slot"),
        1.0
    );
...
fn coupling_refuses_an_unmappable_slot() {
    ...
    assert_eq!(
        couple_prior(&context, &prior, &[(3, 0)]).unwrap_err(),
        qualia_cuda::CudaError::UnknownType(3)
    );
    assert_eq!(
        couple_prior(&context, &prior, &[(1, STATE_DIM)]).unwrap_err(),
