...
| Camera node | **present, multi-reader** | `python3 -c "import os; os.open('/dev/video0', os.O_RDWR)"` | `OPENED fd=3` — *not* `EBUSY`; the UVC node (`0bda:5842` on `/dev/video0`+`/dev/video1`) takes several readers, so D-025's "EBUSY for anyone else" is the two serial ports only (the decision was drafted, and cited in the record, as D-023; corrected in `../T59/devices/README.md`) |
| Robot IMU | **absent from the board's own buses** | `ls /sys/bus/iio/devices`; `i2cdetect -y -r 0`, `-r 1`, `-r 2`, `-r 7` | empty; no responder at `0x68`/`0x69`/`0x76`/`0x28`/`0x6a`/`0x6b` — the robot's inertial data exists only as the leash's `sensors.imu` (`leash surface imu=available(waveshare-ugv)`) and no runner consumes it yet (T59, [#240](https://github.com/superposition/qualia/issues/240)) |
