...
`capture.json`, the README, the derived tables and their generator (a `timeline.csv` and the script
that writes it, `docs/evidence/T31/training-step/make_timeline.py`), the captured binary's own
progress output when it emits one (`train-progress.txt`), and the backend export **trimmed** to the
tables that timeline is read from; its README carries the timeline numbers, states the trim's
...
ticket from there. A capture on the dev host is not the convention. Two committed cases are the
carve-outs: a kernel-less CPU timeline — it opens no CUDA device for the board's `ncu`, and Pinkie
carries no `nsys` to trace it there (T35 and T31) — and a run whose package the board cannot execute,
T50's model step needing `candle-core/cuda`, which the board's offline registry cache lacks and its
DNS-less host cannot fetch. Each may be captured on the dev host, accepted on precedent. The host's
WSL2 Ubuntu-22.04 distribution is where mage installs on this workstation (`triton>=3.0`
