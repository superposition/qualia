...
that writes it, as T47's `make_timeline.py` does) and the backend export **trimmed** to the tables
that timeline is read from; its README carries the timeline numbers, states the trim's
...
ticket from there. A capture on the dev host is not the convention. A kernel-less CPU timeline is the
one carve-out: it opens no CUDA device and Pinkie carries no `nsys` for it, so it may be captured on
the dev host — T35 and T31 did, accepted on precedent. The host's WSL2 Ubuntu-22.04
