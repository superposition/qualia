...
**The claim.** The released FlyEM male-CNS connectome — **166,700 neurons, 25,582,938 neuron-level
edges and 124,177,617 synapses**, imported from the public v1.0 tables under CC-BY 4.0 (#236;
`docs/evidence/T55/connectome-runner/README.md`) — now steps as a spiking network on the robot's own
computer: **11.840 ms/tick, 84.5 ticks/s** and 2,160.6 M synapse-updates/s on Pinkie's Orin NX iGPU at
182,247,874 B resident, 1.41× inside a 60 Hz frame, and **1.381 ms/tick, 18,519.0 M synapses/s** on the
development RTX 4090. The same network is drawn inside the operator's console from the committed
artifact, with the runner's real firing set (**200 frames, 10,048,960 firing ids**; `spikes.bin`
40,199,848 B, sha256 `a0a27acb…3b28`; #237). The robot's own sensors reached the dataset and the
trainer: a 900 s capture of the leash's camera and LD06 into one MCAP (**83,299,459 B**, sha256
`97612acb…3334`) formed 4,501 candidate transitions of which **0 were valid**, every one rejected
`calibration_missing` (#239); the calibration, pose and leash-authority applied-action producers added
in #250 took a 300 s session (30,300,801 B) to **`valid=1499 candidates=1500`**, and the trainer then
refused at its named floor — `dataset does not meet the 50k/12-session/3-condition/3-environment gate`
(D-027). A DeepSeek call through the mission broker produced a decision the agent accepted with
**HTTP 202** (#244), and the review of that status surface found four characters of the live key in it;
#251 removed prefix mode (`key=<present via DEEPSEEK_API_KEY_FILE>`). The four Python gates are now one
Rust binary whose six output streams are byte-identical to the scripts (#238), and the hardware audit
measured why the drive path stops where it does: the leash owns both serial ports, so `qualia-lidar`
and `qualia-drive` exit 2 with `Device or resource busy` (#240).
...
CUDA path uses; the device numbers were never affected (#236).
...
the entry quotes the observed loop rate with its conditions (#236, review note N5).
...
LIF kernel became a single kernel (#236). A board build of a *different* package subset re-unifies
features and recompiles the shared closure, which is why the T58 legs cost ~20 min rather than ~4
(#239).
...
* **`crates/gates`** (`qualia-gates`): the four gates — `provenance`, `figures`, `journal`, `mission` —
  as one Rust binary, with the Python scripts deleted and no shim; the parity evidence is six
  identical output streams plus matching exit codes (#238).
...
* **`runners/calibration` and `runners/leash-transport`**: the calibration document whose
  ...
  `authority=leash` (#250).
* **`runners/mission-broker`** and the console's Coach panel: the first thing in the tree that calls a
  model, with a `redact` module that names a credential by presence only (#244, #251).
...
| `brain-to-robot.svg` (+ `.png`) | diagram | The path the epic built: the four public release tables into the importer's QLCN artifact (166,700 / 25,582,938 / 124,177,617), the LIF runner at 11.840 ms/tick, the console Brain view, the robot's sensors through the leash's HTTP surface into the encoder (8 columns, gain 0.5), the decoder (2,012 motor neurons) and the blocked drive path, with the leash's ownership boundary dashed, and below it MCAP → dataset gates → the 50k/12/3/3 floor. |
| `wave-numbers.json` | data | Every number the figures draw, each with the committed evidence file it came from. The generator refuses to draw a value this file does not carry. |
...
`docs/evidence/T58/real-sessions/README.md` and `docs/evidence/T65/calibration/README.md`. The floor
above them is `docs/decisions.md` D-027 and is off the chart: 50,000 valid transitions is 33.4× the
whole accepted session, before the 12-session / 3-environment / 3-condition diversity.*

![From the release tables to the robot's wheels](https://raw.githubusercontent.com/superposition/qualia/main/docs/figures/the-fly-brain-on-the-robot/brain-to-robot.svg)

*`brain-to-robot.svg`: the epic end to end. The dashed line is the leash's ownership boundary as
measured in `docs/evidence/T59/devices/README.md` — both serial ports are single-owner (errno 16), so
`qualia-lidar` and `qualia-drive` measure a refusal and exit 2; the camera node takes several readers,
and the stack still subscribes to the leash's HTTP surface rather than opening it.*

### Data

`wave-numbers.json` carries each figure's numbers with the source file it was read from, and
`make_figures.py` refuses to draw a value the file does not carry. The quantities, with units and
sources:
...
| Mission broker | `POST /mission-control/envelopes -> HTTP 202 accepted=true idempotent_replay=false`; requested `deepseek-chat`, provider reported `deepseek-flash`; latency 435 ms | `docs/evidence/T63/README.md` |
| Gate parity | six output streams identical to the frozen Python; `provenance: OK` | #238 braid; PR #253 |
...
**Sources disagree in two places, and both are quoted rather than resolved.** #236's first braid gives
...
same bench, a later pass; the figures use the evidence file's numbers. And that README's log list says
the import took "24.7 s" while its own transcript prints `-> artifact in 26.0445714s`; no figure draws
either.
...
* **The lidar and drive success paths are unexercised on the robot.** The board rows measure the
  *refusal* — `Device or resource busy`, exit 2 — which is the documented contract for a port the
  ...
  without taking a port from the leash, which D-025 rules out.
...
* **This is not yet the published entry.** The figures live beside this README; the site's front matter
  and the live URL are added at the publish step, and the console figures shown here are not the site's
  images.
