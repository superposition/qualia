...
//! shared memory. Publishing waits for a recent belief commit, and only a
//! belief silence past `QUALIA_BELIEF_PACE_MS * 8` lets a pose publish without
//! one.
//!
...
fn main() {
    ...
    let poll_ms = setting(std::env::var("QUALIA_POSE_POLL_MS").ok(), POLL_MS_FALLBACK);
    let pace = Duration::from_millis(poll_ms);
    let report_every = setting(
        std::env::var("QUALIA_POSE_LOG_EVERY_UPDATES").ok(),
        LOG_EVERY_FALLBACK,
    )
    .max(1);
    let belief_pace_ms = setting(
        std::env::var("QUALIA_BELIEF_PACE_MS").ok(),
        DEFAULT_BELIEF_PACE_MS,
    );
    ...
    loop {
        ...
        if matches!(outcome, Outcome::Bootstrapped { .. } | Outcome::Tracked(_)) {
            if let Some(lag_ms) = belief_pace.await_pace(&region, poll_ms) {
