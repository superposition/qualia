...
const BIN: &str = env!("CARGO_BIN_EXE_qualia-l1-belief");
...
fn launch(tag: &str, mode: Option<&str>, prior: Option<&Path>) -> Output {
    ...
    command.output().expect("spawn the l1-belief runner")
}
...
/// on a non-macOS host refuses there, naming its layer.
