...
//! no Metal device the backend refuses to start the layer, which is the one
//! path through this binary that ends rather than running forever — the loop
//! itself is covered by the backend's own tests.
//!
...
fn the_refusal_names_the_layer_the_runner_was_started_for() {
    ...
    assert!(
        stderr.contains("l4-behavior"),
        "the refusal does not name the layer: {stderr}"
    );
    assert!(
        stderr.contains("layer 4"),
        "the refusal does not name the layer index: {stderr}"
    );
}
