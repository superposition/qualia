...
/// One running worker, shared by every test in this binary so that a single
/// CUDA context is opened no matter how many checks run.
struct Service {
    child: Mutex<Child>,
    endpoint: String,
    log_path: PathBuf,
...
impl Service {
    ...
    fn request(&self, request: &Value) -> Value {
        round_trip(&self.endpoint, &serde_json::to_string(request).expect("encode request"))
    }
...
fn service() -> &'static Service {
    static SERVICE: LazyLock<Service> = LazyLock::new(Service::start);
    &SERVICE
}
...
fn the_operator_log_names_the_endpoint_and_the_cuda_runtime() {
    let service = service();
    // A served request proves the worker is past initialization, so both log
