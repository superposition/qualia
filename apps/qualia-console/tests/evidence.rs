...
fn the_scan_follows_partials_as_they_appear_and_vanish() {
    ...
    let empty = scan.refresh(directory.path(), Vec::new());
...
fn an_unreadable_segment_is_named_and_not_kept() {
    ...
    let failed = scan.refresh(directory.path(), Vec::new());
    ...
    let cleared = scan.refresh(directory.path(), Vec::new());
    assert!(
